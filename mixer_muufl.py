...
class MLPMixer(nn.Module):
    ...
    def __init__(self, in_channels, dim, num_classes, patch_size, image_size, depth, token_dim, channel_dim,lidar_size):
        ...
        self.d2_affi_two = nn.Conv2d(in_channels=dim, out_channels=dim, kernel_size=1)
        self.scale = 4
        self.scale_dim = dim * self.scale
        self.depth_kernel = 3
        self.up_proj = nn.Sequential(nn.Linear(dim, self.scale_dim),
                                     Rearrange('b n c -> b c n'),
                                     nn.BatchNorm1d(self.scale_dim),
                                     nn.GELU(),
                                     Rearrange('b c (h w) -> b c h w', h=11, w=11)
                                     )

        self.depth_conv = nn.Sequential(
            nn.Conv2d(self.scale_dim, self.scale_dim, kernel_size=self.depth_kernel, padding=1, groups=self.scale_dim, bias=False),
            nn.BatchNorm2d(self.scale_dim),
            nn.GELU(),
            Rearrange('b c h w -> b (h w) c', h=11, w=11)
        )

        self.down_proj = nn.Sequential(nn.Linear(self.scale_dim, dim),
                                       Rearrange('b n c -> b c n'),
                                       nn.BatchNorm1d(dim),
                                       nn.GELU(),
                                       Rearrange('b c n -> b n c')
    ...
    def forward(self, x, x1):
        ...
        # x_xiu = self.up_proj(x)
        # x_xiu = self.depth_conv(x_xiu)
        # x = self.down_proj(x_xiu)
        ...
        affinity_matrix = torch.sigmoid(affinity_matrix)
        x_all_1 = torch.mul(x_all, affinity_matrix)
        x_out = self.conv_out(x_all_1)
        ...
        x_all_pre = self.conv_mid(x_all_1)
        return x_out,x_all_pre




if __name__ == "__main__":
    img = torch.ones([12, 94, 11, 11])
    img2 = torch.randn([12, 1, 15, 15])
    model = MLPMixer(in_channels=94, image_size=11, patch_size=1, num_classes=6,
                     dim=128, depth=2, token_dim=128, channel_dim=128,lidar_size=15)
    parameters = filter(lambda p: p.requires_grad, model.parameters())
    parameters = sum([np.prod(p.size()) for p in parameters]) / 1_000_000
    print('Trainable Parameters: %.3fM' % parameters)

    out_img,pre = model(img,img2)
