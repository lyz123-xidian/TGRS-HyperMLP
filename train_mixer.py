...
# from single_lidar import MLPMixer
from attributionpriors.pytorch_ops import AttributionPriorExplainer
import time

### parm and dataloader
hsi_datapath = '/home/sd1/liuyuzhe/TRENTO/hyper_Italy.tif'
hsi_datapath1 = '/home/sd1/liuyuzhe/TRENTO/trento_20.mat'
lidar_datapath = '/home/sd1/liuyuzhe/TRENTO/LiDAR_Italy.tif'
train_path = '/home/sd1/liuyuzhe/TRENTO/TNsecSUBS_Train.tif'
test_path = '/home/sd1/liuyuzhe/TRENTO/TNsecSUBS_Test.tif'
num_class = 6
patchsize = 7
patchsize_lidar = 15
batchsize = 24
...
x = np.empty((172, 606, channel), dtype='float32')
...
### model_build
model = MLPMixer(in_channels=93, image_size=7, patch_size=1, num_classes=6,
...
for epoch in range(EPOCH):
    for step, (x1,x2,y) in enumerate(train_loader):
        ...
        if step % 50 == 0:
            ...
            if accuracy > bestacc and epoch>1:
                # torch.save(model.state_dict(), 'trento_weight.pth')
                bestacc = accuracy
            model.train()
...
model.load_state_dict(torch.load('trento_weight.pth'))
model.eval()
torch.cuda.synchronize()
start = time.time()
pred_y = np.empty((len(TestLabel)), dtype='float32')
number = len(TestLabel)//5000
# pre_tsne = np.empty((len(TestLabel),256),dtype='float32')
...
for i in range(number):
    ...
    # pre_fea = model(temp, temp_lidar)[1]
    # pre_fea = pre_fea.detach().numpy()
    temp3 = torch.max(temp3, 1)[1].squeeze()
...
# pred_y = torch.from_numpy(pred_y).long()
# pre_tsne = torch.from_numpy(pre_tsne).long()
pre_tsne = pre_tsne[5000:,:]
savemat('matrix_tre_mlp.mat',{'tsne':pre_tsne})
savemat('matrix_tre_mlp_label.mat',{'label':pred_y})
pred_y = torch.from_numpy(pred_y).long()
...
pred_final = pred_map
cv2.imwrite('pred_trento.png',pred_final)
np.save('pred_trento', pred_final)
