...
from mixer_hou import MLPMixer
import time
from scipy.io import savemat
### parm and dataloader
hsi_datapath = '/home/sd1/liuyuzhe/Two-branch-CNN-Multisource-RS-classification-master/data/houston/2013_IEEE_GRSS_DF_Contest_CASI.tif'
hsi_datapath1 = '/home/sd1/liuyuzhe/hou13/houston_50.mat'
train_path = '/home/sd1/liuyuzhe/Two-branch-CNN-Multisource-RS-classification-master/data/houston/Houston_train.tif'
test_path = '/home/sd1/liuyuzhe/Two-branch-CNN-Multisource-RS-classification-master/data/houston/Houston_test.tif'
lidar_datapath = '/home/sd1/liuyuzhe/Two-branch-CNN-Multisource-RS-classification-master/data/houston/2013_IEEE_GRSS_DF_Contest_LiDAR.tif'
num_class = 15
patchsize = 11
patchsize_lidar = 15
batchsize = 24
...
# hsi_data = sio.loadmat(hsi_datapath)
hsi_data = io.imread(hsi_datapath)
hsi_data_pca = sio.loadmat(hsi_datapath1)
hsi_data_pca = hsi_data_pca['pca_data']
# lidar_data = sio.loadmat(lidar_datapath)
lidar_data = io.imread(lidar_datapath)
# train_label = train_label['trainlabels']
# test_label = test_label['testlabels']
# hsi_data = hsi_data['hsi']
# lidar_data = lidar_data['lidar']
hsi_data_pca = hsi_data_pca.astype(np.float32)
hsi_data = hsi_data.astype(np.float32)
lidar_data = lidar_data.astype(np.float32)

###   data normlization
[m, n, l] = hsi_data.shape
[m1,n1,l1] = hsi_data_pca.shape
for i in range(l):
...
for i in range(l1):
    minimal = hsi_data_pca[:,:,i].min()
    maximal = hsi_data_pca[:,:,i].max()
    hsi_data_pca[:,:,i] = (hsi_data_pca[:, :, i] - minimal) / (maximal - minimal)
hsi_data = np.concatenate((hsi_data,hsi_data_pca),axis=2)
x = np.empty((359, 1915, channel), dtype='float32')
...
### model_build
model = MLPMixer(in_channels=174, image_size=11, patch_size=1, num_classes=15,
...
for epoch in range(EPOCH):
    for step, (x1,x2,y) in enumerate(train_loader):
        ...
        if step % 50 == 0:
            ...
            if accuracy > bestacc:
                torch.save(model.state_dict(), 'houston_weight.pth')
                bestacc = accuracy
            model.train()
...
model.load_state_dict(torch.load('houston_weight.pth'))
model.eval()
torch.cuda.synchronize()
start = time.time()
pred_y = np.empty((len(TestLabel)), dtype='float32')
...
savemat('matrix_hou_mlp.mat',{'tsne':pre_tsne})
savemat('matrix_hou_mlp_label.mat',{'label':pred_y})
pred_y = torch.from_numpy(pred_y).long()
...
pred_final = pred_map
cv2.imwrite('pred_ssmlp.png',pred_final)
np.save('pred_ssmlp', pred_final)
